...
// data
import { Quiz, next13QuizData } from "@/data/quiz/next/data";
...
export const Board = () => {
  ...
  const shuffled = useShuffle(next13QuizData);
  ...
  const handleNextQuestion = () => {
    ...
    if (currentIndex < next13QuizData.length - 1) {
  ...
  return (
    <div className="">
      <p className="text-right text-xs font-semibold text-gray-500">
        問題 {currentIndex + 1} / {next13QuizData.length}
      </p>
      {isFinished ? (
        <Score quizData={next13QuizData} score={score} />
      ) : (
        <div>
          {feedback ? (
            <>
